...
```
exec python $TICKFLOW_ASSIST_ROOT/scripts/add_stock.py --symbol {股票代码} --cost {成本价}
```
...
```
exec python $TICKFLOW_ASSIST_ROOT/scripts/remove_stock.py --symbol {股票代码}
```
...
```
exec python $TICKFLOW_ASSIST_ROOT/scripts/fetch_klines.py --symbol {股票代码}
```
...
```
exec python $TICKFLOW_ASSIST_ROOT/scripts/list_watchlist.py
```
...
**步骤二** - 调用 LLM 分析：
```
exec python $TICKFLOW_ASSIST_ROOT/scripts/analyze.py --symbol {股票代码}
```
...
```
exec python $TICKFLOW_ASSIST_ROOT/scripts/update_all.py
```
...
```
exec python $TICKFLOW_ASSIST_ROOT/scripts/realtime_monitor.py &
```
...
- 所有脚本都通过 `$TICKFLOW_ASSIST_ROOT/scripts/` 调用，避免写死设备相关路径
