...
当用户请求验证当前告警通道是否可用时（如："测试告警"、"发一条测试消息"、"检查告警通道"），执行：

```
exec uv run python $TICKFLOW_ASSIST_ROOT/scripts/test_alert.py
```

- 发送一条测试消息到当前配置的 channel
- 用于验证 OpenClaw 到目标通道的投递链路
