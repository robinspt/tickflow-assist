...
- 对齐妙想官方数据与智能选股 skill：新增 `mx_data` 官方金融数据查询工具，增强 `mx_select_stock` 对官方返回结构和 Markdown 表格的兼容解析，并在官方忽略 `pageSize` 时由工具层截断展示。
- 新增智能选股候选池联动：`screen_stock_candidates` 先用妙想智能选股生成候选，再按 TickFlow API Key Level 受控补充行情、少量日 K、可选分钟 K 和可选 Expert 财务；新增 `/ta_screenstocks` 与 `/ta_screenstocks_llm`，后者支持 `summarize=true` 追加 LLM 整理。
- 优化盘前资讯简报：重大要闻改为从快讯正文主题点提炼宏观要点，避免多个栏目只围绕自选命中新闻重复输出；同时复核 OpenClaw `v2026.4.22` 兼容与社区元数据。
- 修复实时监控阶段提醒重复发送：阶段提醒改走 `alert_log` 持久化去重，并收紧补偿窗口逻辑，避免状态回写竞争或进程重启后重复推送。

## v0.3.7 - 2026-04-16

- 接入 TickFlow 标的池行业管线与本地缓存，行业资料改为 `TickFlow 优先、妙想兜底`；收盘复盘补充申万三级同业强弱与行业位置，`Free` 等级仍保留降级路径。
- 优化收盘复盘与行情展示：修复 TickFlow `change_pct` 单位误用，统一复盘/分析/监控链路的百分比换算，并在复盘消息头与大盘/板块段落中补充收盘价、当日涨跌幅、成本价和同业位次信息。
- 强化盘前资讯简报正文提炼：先从“金十数据整理”中抽取摘要与要点，再交给 LLM 生成；若输出退化成只抄标题，则自动回退到确定性摘要。
- 回退 Telegram 图文分开发送实验，恢复为图文一起发送；同时同步 OpenClaw `2026.4.14` 兼容、社区发布元数据与 README 顶部发布说明。
...
- 修复 `setup-tickflow.sh` 在 OpenClaw `2026.4.11` 下源码升级/重装时本地链接扫描 `node_modules` 超过目录上限而失败的问题；首次安装改为先注册链接，升级时若已链接到同一路径则跳过重复 link install。
- 插件配置新增环境变量回退：支持为 TickFlow、LLM、MX Search、Jin10 等核心字段通过环境变量提供值；`configure-openclaw` 可保留空值以使用环境变量，并支持输入 `-` 清空已写入的密钥配置。
