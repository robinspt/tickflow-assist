...
最近更新：`v0.3.10` 跟进 OpenClaw `v2026.6.10`：新版复核未发现需要调整插件运行逻辑的破坏性变化，开发期 SDK 与构建元数据已同步到 `2026.6.10`，告警投递继续统一走 `openclaw message send --json`。完整发布记录见 <https://github.com/robinspt/tickflow-assist/blob/main/CHANGELOG.md>。

当前主线最低兼容 OpenClaw `v2026.5.2+`，当前开发主线已按 OpenClaw `v2026.6.10` SDK / 插件清单口径复核，并使用 `contracts.tools` 与 `setup.providers[].envVars` 等新版插件清单字段。建议 Node `>=22.19.0`，并以目标 OpenClaw 版本上游要求为准。
