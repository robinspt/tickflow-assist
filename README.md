...
最近更新：`v0.3.10` 跟进 OpenClaw `v2026.5.12`：构建口径与 Node 要求同步到新版，告警投递统一走 `openclaw message send --json`，可选工具补充 `toolMetadata.optional`，并降低 ClawHub 扫描误报风险。当前开发主线已复核 OpenClaw `v2026.5.18`。完整发布记录见 [CHANGELOG.md](CHANGELOG.md)。
...
- TickFlow Assist 当前主线最低兼容 OpenClaw `v2026.5.2+`，当前开发主线已按 OpenClaw `v2026.5.18` SDK / 插件清单口径复核，并使用 `contracts.tools` 与 `setup.providers[].envVars` 等新版插件清单字段
- 建议 Node `>=22.19.0`，并以目标 OpenClaw 版本上游要求为准
...
- 更新提醒：可在北京时间 `21:00` 自动检查 npm / GitHub Release 版本，发现新版本后通过告警通道推送一次。
...
向导也会 best-effort 安装 OpenClaw 媒体发送所需的全局 `sharp`，并在 Linux 上尝试安装 PNG 告警卡所需的中文字体。
...
后台任务统一由 `tickflow-assist.managed-loop` 托管，在同一个 service 内并行运行日更、实时监控、金十数据 24 小时快讯监控与自动版本检查。
...
- `sharp`：插件本地依赖用于生成 PNG；OpenClaw `message send --media` 还需要宿主 OpenClaw 全局 npm 环境可解析 `sharp`，一键脚本会 best-effort 执行 `npm install -g sharp`。
