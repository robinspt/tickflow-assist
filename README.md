...
- 收盘后日更新增综合分析、活动价位历史快照与自动回测，并提供 `backtest_key_levels` 工具。
- 关键价位回测补充 `1/3/5` 日支撑、压力、止损、止盈、突破统计；同日双触发时优先用分钟线判定先止损还是先止盈。
...
- 收盘后复盘：日更后自动执行综合分析、刷新活动价位，并输出回测摘要；复盘摘要会在后续分析中自动引用
- 关键价位回测：支持 `1/3/5` 日支撑、压力、止损、止盈、突破统计，同日双触发优先用分钟线判定先后
...
- 实时监控：按交易时段轮询报价并触发告警
- 定时日更：交易日 15:25 后执行，更新自选股票日K、分钟K，并附带收盘后分析与回测结论
...
- `update_all`：收盘后批量更新日K、日线指标和当日分钟K，并追加活动价位回测摘要
- `analyze`：读取本地日线数据，并临时补充分钟线、分钟指标、实时行情与历史复盘摘要做综合分析
- `backtest_key_levels`：回测活动价位快照，输出 `1/3/5` 日支撑、压力、止损、止盈、突破与路径统计
