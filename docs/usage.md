...
复杂条件可以直接用完整自然语言描述，例如“沪深A股中今日涨幅大于2%，市盈率小于30，市净率小于3，净利润同比增长为正，总市值小于300亿，属于半导体、人工智能或机器人相关板块的股票”。如需在候选池后追加模型整理，可以使用 `/ta_screenstocks_llm` 或直接说明“用 LLM 整理候选”。
...
插件当前注册了 20 个 `/ta_` 直达命令，适合“添加自选”“查状态”“发测试告警”这类不需要模型解释的操作。
...
| 命令 | 说明 |
|---|---|
| `/ta_screenstocks <自然语言选股条件>` | 智能选股并生成小规模候选池，不调用 LLM 整理 |
| `/ta_screenstocks_llm <自然语言选股条件>` | 智能选股并生成小规模候选池，再调用 LLM 做候选整理 |
...
| 智能选股 Slash Command | `/ta_screenstocks <自然语言选股条件>` 生成结构化候选池；`/ta_screenstocks_llm <自然语言选股条件>` 在候选池后追加 LLM 整理 |
