...
def calculate_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    返回新 DataFrame，包含原始列 + 指标列
    """
    if len(df) < 5:
        raise ValueError("K线数据不足，至少需要5根K线")
    ...
    # RSI (6, 12, 24)
    # ============================================================
    result["rsi_6"] = ta.momentum.RSIIndicator(
        close=result["close"], window=6
    ).rsi()
    result["rsi_12"] = ta.momentum.RSIIndicator(
        close=result["close"], window=12
    ).rsi()
    result["rsi_24"] = ta.momentum.RSIIndicator(
        close=result["close"], window=24
    ).rsi()
